<template>
  <div>
    ...
    <Modal v-model="dialog.show" title="title" :mask-closable="false">
      <div slot="header">
        <h3 v-if="dialog.type === 'add'">添加菜单组</h3>
        <h3 v-if="dialog.type === 'modify'">修改菜单组</h3>
      </div>
      <div>
...
<script>
  ...
  export default {
    ...
    methods: {
       ...
       */
      getList (page) {
        Service.getList(page, this.filterData).then(result => {
          if (result.isSuccess) {
      ...
      updateAdd () {
        this.$refs['form'].validate((valid) => {
          if (valid) {
            ...
            Service.add(this.dialog.form.data).then(result => {
